...
app.post("/signup",(req,res)=>{
  ...
  Auth.findOne({name:namee},async (err,foundP)=>{
    if(foundP){
...
res.render('personalize',{title:"",blog:""})
})
app.post("/personalize",(req,res)=>{
  const t = req.body.search;
  
  Blog.find({title:t},(err,foundBlog)=>{
    if(foundBlog.length>0){
      res.render("personalize",{title:foundBlog[0].title,blog:foundBlog[0].blog})
      
    }
    else{
      res.render("personalize",{title:"No such Blog",blog:""})
    }
  })
})
